import type { Slide } from "./about-carrousel";
import { SwiperSlide } from "swiper/react";

interface Props {
  slide: Slide;
}

export const AboutCarrouselSlide = ({ slide }: Props) => {
  return (
    <SwiperSlide key={slide.media_path}>
      <div className="flex flex-col items-center mb-20">
        <h2 className="font-arvo text-3xl mb-10">{slide.title}</h2>
        <div className="relative overflow-hidden h-93 max-w-91 mx-auto rounded-lg">
          <video
            className="object-cover absolute object-[center_-60px] left-0 top-0"
            src="/assets/videos/about_character.mp4"
            autoPlay
            muted
            loop
            playsInline
          />
        </div>
      </div>
    </SwiperSlide>
  );
};
